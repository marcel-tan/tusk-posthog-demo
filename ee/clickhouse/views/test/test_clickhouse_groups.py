...
class GroupsViewSetTestCase(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_upsert_properties_add_success(self, mock_capture):
        ...
        self.assertEqual(response.json()["results"][1]["detail"]["changes"][0]["action"], "created")
        self.assertEqual(response.json()["results"][1]["detail"]["changes"][0]["before"], None)
    ...
    def test_upsert_properties_update_success(self, mock_capture):
        ...
        self.assertEqual(len(response.results[0]), 1)
        self.assertEqual(
            orjson.loads(response.results[0][0]), {"name": "Mr. Krabs", "industry": "technology", "plan": "enterprise"}
        )
